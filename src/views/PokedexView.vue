...
// Datos filtrados por tipo o generación
...
// Cargar tipos y generaciones
const loadFilters = async () => {
  try {
    const [t, g] = await Promise.all([
      pokeService.getTypes(),
...
const filterByType = async () => {
  ...
  try {
    ...
    // Cargar detalles de los primeros 40
...
const displayedPokemon = computed(() => {
  ...
  // Filtrar por búsqueda local (dentro de la lista cargada)
  ...
  <div class="app-container" style="padding-bottom:3rem;">
    ...
    <div class="search-wrapper">
      ...
      <div class="search-box">
        ...
        <button @click="doSearch" class="pagination__btn" style="margin:0.3rem;padding:0.5rem 1rem;font-size:0.7rem;">
          BUSCAR
        </button>
      </div>
    </div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
      ...
      <div style="display:flex;gap:0.5rem;align-items:center;">
        ...
        <button
          ...
          :style="sortBy === 'number' ? 'background:var(--neon-blue);color:#000;' : ''"
          style="font-size:0.6rem;padding:0.3rem 0.8rem;"
        >#NUM</button>
        <button
          @click="sortBy = 'name'"
          class="pagination__btn"
          :style="sortBy === 'name' ? 'background:var(--neon-blue);color:#000;' : ''"
          style="font-size:0.6rem;padding:0.3rem 0.8rem;"
        >A-Z</button>
      </div>
    </div>
    ...
    <div v-else-if="displayedPokemon.length === 0" class="empty-state">
      ...
      <p class="empty-state__text">Intenta con otro nombre, número o filtro</p>
    </div>
    ...
    <div v-else class="poke-grid">
      ...
      >
        ...
        <button
          class="poke-card__fav-btn"
          @click.stop="toggleFav(pokemon)"
          :title="favoritesStore.isFavorite(pokemon.id) ? 'Quitar' : 'Agregar'"
        >
    ...
    <!-- Pagination (solo si no hay filtro activo) -->
    <div v-if="!loading && !searchQuery && filteredByApi === null" class="pagination">
      ...
      <button class="pagination__btn" @click="nextBatch">
